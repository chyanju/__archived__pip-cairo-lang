...
def encode_instruction(element: BytecodeElement, prime: int) -> List[int]:
    ...
    assert (
        -(2 ** (OFFSET_BITS - 1)) <= inst.off1 < 2 ** (OFFSET_BITS - 1)
    ), f"off1 must be in range [-2**{OFFSET_BITS - 1}, 2**{OFFSET_BITS - 1})"
    assert (
        -(2 ** (OFFSET_BITS - 1)) <= inst.off2 < 2 ** (OFFSET_BITS - 1)
    ...
    assert (inst.imm is not None) == (
        inst.op1_addr is Instruction.Op1Addr.IMM
...
def is_call_instruction(encoded_instruction: int, imm: Optional[int]):
    ...
    try:
        ...
        return False
    return (
        instruction.res is Instruction.Res.OP1
        and instruction.pc_update in [Instruction.PcUpdate.JUMP, Instruction.PcUpdate.JUMP_REL]
        and instruction.ap_update is Instruction.ApUpdate.ADD2
        and instruction.fp_update is Instruction.FpUpdate.AP_PLUS2
        and instruction.opcode is Instruction.Opcode.CALL
    )
